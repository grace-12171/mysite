...
def blog_posts():
    ...
    return render_template("post.html", post_data=data)
...
def create():
    ...
    if request.method == 'POST':
        ...
        insert_page = "INSERT INTO posts (title, content, author_id) VALUES (?, ?,?)"
        db.execute(insert_page, (title, content, user_id))
        db.commit()
...
def login():
    ...
    '''      
        ...
        #login (sessions)
        print("attempt to log user in right here")
        return redirect(url_for("index"))
    '''
...
def signup():
    if request.method == 'POST':
        ...
        db.execute(query, (username, generate_password_hash(password, method = 'pbkdf2')))
        db.commit()
